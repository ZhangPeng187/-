...
TimerNode::TimerNode(SP_ReqData request_data, int timeout)
    : deleted(false), request_data(request_data)
{
    // cout << "TimerNode()" << endl;
    struct timeval now;
...
bool TimerNode::isvalid()
{
    ...
    gettimeofday(&now, nullptr);
    //!!!!!!!!!!!!!!!!!
    size_t tem = (now.tv_sec * 1000) + (now.tv_usec / 1000);
    if (tem < expired_time)
        return true;
    this->setDeleted();
    return false;
...
void TimerManager::addTimer(SP_ReqData request_data, int timeout)
{
    ...
    {
        unique_lock<mutex> uqlock(lock);
        TimerNodeQueue.emplace(new_node);
    }
